var YDataGrid = function(config){
	...
	var Action = {
			...
			'exports': actionUrl.exports||'export.do',
			'import': actionUrl.imports||'import.do',
			'downFile': actionUrl.downFile||'downFile.do'
	}
	...
	var Handler = {
			...
			remove: function(callback){
				...
				for(var i=0;i<records.length;i++){
					if(records[i].usenum != ''&&records[i].usenum != null){
						YiYa.alert('警告','已使用的单号不能删除.','warning');
						return false;
					}
				}
				if (Utils.checkSelect(records)){
					$.messager.confirm('系统提示','确定要删除该条数据?',function(r){  
						if (r){
							var idKey = dataGrid.idField || 'id'; //主键名称
			...
			close : function (callback){
				$.messager.confirm('系统提示','确定要关闭该窗口吗?',function(r){  
					if (r){  
						Win.edit.dialog('close');
						//Grid.datagrid('reload',param);
						//回调函数
			...
			exports :function (callback){
					var  data = Form.search.serializeObject(); //序列化字段{'importnum':Form.search.importnum,'importuser':Form.search.importuser}; //序列化字段
					if(data.importnum==''||data.importuser==''){
						YiYa.alert('提示','中英品名转换导出时，总运单号及导入用户不能为空!'); 
						return;
					}
					$.messager.confirm('系统提示','确定要导出文件吗?',function(r){
						if(r){
							YiYa.progress('请等待','导出中...');
							YiYa.ajaxJson(Action.exports,data,function(result){
								YiYa.closeProgress();
								if(result.success){
									YiYa.fileForm(Action.downFile,result.filepath,result.filename);
									//回调函数
									if(jQuery.isFunction(callback)){
										callback(result);
									}
			...
			imports :function (callback){
				$("#multiple_file_upload").uploadify({
					...
					'width'         : 70,  
					'buttonText'    : '浏 览',
					'swf'           : '/ms/swf/uploadify.swf',
					'uploader'      : Action.import,
					'cancelImg'     : '/ms/icon/cancel.png',
					'auto'          : false,
	...
	var Events ={
			...
			//导出按钮事件
			exports : evt.exports || Handler.exports,
			//导入菜单按钮事件
			imports : evt.imports || Handler.imports,
			//导入按钮事件
			import : evt.import || Handler.import
	}
	...
	var bar_imports = { 
			...
			iconCls:'icon-save',
			btnType:'imports',
			handler:Events.imports
	};
	var toolbarConfig = [bar_add,bar_edit,bar_remove,bar_exports];
	var getToolbar = function (){
		...
		if (dataGrid.toolbar != undefined && dataGrid.toolbar.length > 0) {
			for ( var i = 0; i < dataGrid.toolbar.length; i++) {
				...
				if(bar.btnType=='exports'){
					tbars.push({id:bar.id || bar_exports.id,text:bar.text || bar_exports.text ,iconCls: bar.iconCls || bar_exports.iconCls,btnType: bar.btnType || bar_exports.btnType,handler:bar.handler || bar_exports.handler});
					continue;
				}
				if(bar.btnType=='imports'){
					tbars.push({id:bar.id || bar_imports.id,text:bar.text || bar_imports.text ,iconCls: bar.iconCls || bar_imports.iconCls,btnType: bar.btnType || bar_imports.btnType,handler:bar.handler || bar_imports.handler});
					continue;
				}
	...
	var initWin = function(){
		...
		if(Win.file && Win.file[0]){
			var btns = Win.file.attr("buttons");
			if(!btns){
				//设置 导入按钮
				Win.file.dialog({
					buttons:[
